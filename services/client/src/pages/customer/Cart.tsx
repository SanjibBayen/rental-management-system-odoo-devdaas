...
import { Trash2, ArrowRight, ShieldCheck, CheckCircle2, Lock, CreditCard, ShoppingCart } from 'lucide-react';
...
export default function Cart({ setActiveView }: { setActiveView: (view: string) => void }) {
  ...
  const grandTotal = ((totalPrice * 1.18) + 500).toFixed(2);

  const handleInitiatePayment = () => {
    setIsRazorpayOpen(true);
  };

  const handleRazorpaySuccess = (e: React.FormEvent) => {
    e.preventDefault();
  ...
  if (isSuccess) {
    return (
      <div className="w-full max-w-7xl mx-auto px-margin-desktop py-20 text-center flex flex-col items-center justify-center animate-fade-in-up">
        ...
        <h2 className="text-4xl font-black text-on-surface mb-4">Payment Successful!</h2>
        <p className="text-lg text-on-surface-variant mb-8 max-w-md">Your order has been confirmed and is being prepared for dispatch. You will be redirected to your rentals shortly.</p>
    ...
    <div className="w-full max-w-7xl mx-auto px-margin-desktop py-8 animate-fade-in-up">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-4">
          ...
          {items.map((item) => (
            <div key={item.id} className="bg-white rounded-xl border border-border-standard p-4 flex flex-col sm:flex-row gap-6 items-start sm:items-center shadow-sm hover:shadow-md transition-shadow">
              ...
              <div className="flex-1">
                <h3 className="font-bold text-on-surface leading-tight text-lg mb-1">{item.name}</h3>
                <div className="text-outline text-sm font-bold uppercase tracking-wider mb-3">{item.brand}</div>
                <div className="flex items-end gap-2">
                  <div className="text-primary font-black text-2xl">₹{item.pricePerDay}</div>
                  <div className="text-sm text-outline font-medium mb-1">/ day</div>
                </div>
              </div>
        ...
        <div className="bg-white rounded-2xl border border-border-standard p-8 h-fit sticky top-24 shadow-lg shadow-surface-muted">
          ...
          <div className="space-y-4 mb-6 text-sm">
            ...
            <div className="flex justify-between text-on-surface-variant font-medium">
              <span>Tax (18%)</span>
              <span className="font-bold text-on-surface">₹{(totalPrice * 0.18).toFixed(2)}</span>
            </div>
            <div className="flex justify-between text-on-surface-variant font-medium">
      ...
      <Modal isOpen={isRazorpayOpen} onClose={() => !isProcessing && setIsRazorpayOpen(false)} title="Secure Payment">
        <form onSubmit={handleRazorpaySuccess} className="space-y-6">
          <div className="flex items-center justify-between p-4 bg-surface-muted rounded-xl border border-border-standard">
            <div className="font-bold text-on-surface flex items-center gap-3">
              ...
              <div className="flex flex-col">
                ...
                <span className="text-[10px] text-outline uppercase tracking-widest mt-1">Test Environment</span>
              </div>
            </div>
            <div className="text-primary font-black text-2xl">₹{grandTotal}</div>
          </div>

          <div className="space-y-4">
            <div className="flex items-center justify-between mb-2">
              <label className="text-sm font-bold text-on-surface">Card Details</label>
            ...
            <div className="border border-border-standard rounded-xl overflow-hidden shadow-sm focus-within:border-primary focus-within:ring-1 focus-within:ring-primary transition-all">
              <input required type="text" placeholder="Card Number" className="w-full px-4 py-3 border-b border-border-standard outline-none bg-white font-mono text-sm placeholder:text-outline/50" maxLength={19} />
              <div className="flex">
                <input required type="text" placeholder="MM/YY" className="w-1/2 px-4 py-3 border-r border-border-standard outline-none bg-white font-mono text-sm placeholder:text-outline/50" maxLength={5} />
                <input required type="text" placeholder="CVC" className="w-1/2 px-4 py-3 outline-none bg-white font-mono text-sm placeholder:text-outline/50" maxLength={3} />
              </div>
            </div>
            
            <div>
              <label className="block text-sm font-bold text-on-surface mb-2">Cardholder Name</label>
              <input required type="text" placeholder="Name on card" className="w-full px-4 py-3 border border-border-standard rounded-xl outline-none focus:border-primary focus:ring-1 focus:ring-primary shadow-sm text-sm" />
            </div>
          </div>
          ...
          >
            <Lock className="w-4 h-4" /> Pay ₹{grandTotal}
          </button>
