...
const ProductCard = memo(function ProductCard({ 
  ...
  const displayBadge = useMemo(() => {
    ...
    if (isLowStock) {
      return { text: `Only ${product.stockQuantity} left`, type: 'warning' as const };
    }
    if (product.rating > 4.5) {
      return { text: 'Top Recommended', type: 'success' as const };
    }
    return product.badge || null;
  }, [product.rating, product.badge, isOutOfStock, isLowStock, product.stockQuantity]);
  ...
  const handleRentNow = useCallback(async (e: React.MouseEvent) => {
    e.stopPropagation();
    e.preventDefault();
  ...
  return (
    ...
    >
      ...
      <div className="relative h-48 w-full bg-surface-container-low overflow-hidden flex items-center justify-center p-4">
        ...
        {/* Badges */}
        {displayBadge && (
          <div className={`absolute top-3 left-3 font-bold text-[10px] uppercase tracking-wider px-2.5 py-1 rounded-full shadow-sm z-10 ${badgeStyle}`}>
            {displayBadge.text}
          </div>
        )}
          ...
          <div>
            ...
            <div className="flex items-baseline gap-1">
              <span className="font-bold text-2xl text-primary">
                {product.pricePerDay > 0 ? formatPrice(product.pricePerDay) : 'Free'}
              </span>
