...
// Extracted components outside to prevent recreation on every render
...
const MobileNavItem = ({ 
  ...
  <button 
    ...
    className={`w-full text-left px-4 py-3 text-sm font-bold transition-colors ${
      variant === 'danger'
        ? 'text-danger-red hover:bg-danger-red/10 border-l-4 border-transparent'
        : isActive 
...
export default function Navbar({ activeView, setActiveView }: NavbarProps) {
  ...
  const handleSearch = useCallback((e: React.FormEvent) => {
    ...
    if (searchQuery.trim()) {
      ...
      // You might want to pass search query via URL params or context
